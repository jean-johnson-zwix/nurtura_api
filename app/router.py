...
from schemas import CreateUserProfileRequest, CreateUserProfileResponse, Response, UserLogin
import crud
...
@router.post('/login')
async def login(req:UserLogin, db:Session=Depends(get_database_session)):
    ##return {"message":"function started"}
    ##_user_profile=crud.userlogin(db,req)
    ##response = CreateUserProfileResponse(user_name=_user_profile["user_name"], user_id=_user_profile["user_id"]) ##Modify this should the code be successful.
    return crud.userlogin(db,req)
